...
function queueDragHandle(item) {
  ...
  handle.setAttribute(
    "aria-label",
    `Reorder ${item.track ? formatting.trackTitle(item.track) : "unavailable track"}`,
  );
...
function handleQueueReorderKey(event, queueItemID) {
  ...
  submitQueueReorder(queueItemID, beforeQueueItemID).then(() => {
    queueEl
      .querySelector(`[data-queue-item-id="${queueItemID}"] .queue-drag-handle`)
      ?.focus();
  });
}
...
function renderQueueChanges(actions) {
  ...
  queueChangesButton.textContent = actions.length
    ? `Queue changes ${actions.length}`
    : "Queue changes";
  queueChangesListEl.replaceChildren(
    ...actions.map((action) => {
  ...
  setQueueSortable(
    Sortable.create(queueEl, {
      ...
      onEnd(event) {
        ...
        const beforeQueueItemID = before
          ? Number(before.dataset.queueItemId)
          : 0;
...
export default {
  ...
  applyPendingQueueState,
  submitQueueReorder,
  formatActionTime: formatting.formatActionTime,
};
